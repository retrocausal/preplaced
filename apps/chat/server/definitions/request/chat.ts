...
export const chatListRequestSchema = z.object({
  userId: z.string({ error: 'User ID is required' }).nonempty('User ID must be non-empty'),
  limit: z.number().int().min(1, 'Limit must be a positive integer').optional().default(10),
